	...
	stage("Deploy to staging") {
		...
		container('gke-deploy') {
		    sh "sed -i s#IMAGE#${GCR_IMAGE}#g kubernetes/manifest.yaml"
                    step([$class: 'KubernetesEngineBuilder', projectId: capsgcppoc, clusterName: env.STAGING_CLUSTER, location: env.PROJECT_ZONE, manifestPattern: 'kubernetes/manifest.yaml', credentialsId: env.JENK_INT_IT_CRED_ID, verifyDeployments: true])
		}
            }
	}
        /**
